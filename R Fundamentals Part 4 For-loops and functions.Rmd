...
This is a [.HTML](https://html.com/) file generated using R markdown (.Rmd). Check out [the R markdown pages](http://rmarkdown.rstudio.com/) (yihui)[https://yihui.name] to learn more.  

**Learning objectives review - Part 1:**  
...
3. ggplot2: layers; _three parts: data, aes, geom_  
...
Let's start simple! An easy for loop could look like this: 
```{r}
for(x in 1:10){
  print(x)
}
...
For long loops, many intermediate commands called "control structures" can help us "end the looping" once a condition is met, or when a condition is no longer satisfied. Two particular useful ones are `break` and `stopifnot`.  

`break` ends the looping once a certain condition is met.

`stopifnot` ends the loop when a certain condition is NOT met.  

Let's begin by writing a for loop that outputs the numbers of the sequence 1:10  
```{r, eval=FALSE}
for(i in 1:10){
    print(i) 
    }
...
##### 2.3.1 For-loops (`if`)
`if` statements are frequently used to specify code to be evaluated when the condition is held. Here we tell the code that when `i` is equal to 5, stop the code.  
...
We usually have data that we pass into a for loop. Let's create an object called `my_nums` that contains the numeric vector 40:70:
```{r}
my_nums <- 40:70
...
Let's quickly amend our `my_nums` variable to include one `NA` in position 15: 
```{r}
...
You can even use columns from a dataframe:
```{r}
