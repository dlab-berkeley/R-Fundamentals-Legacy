...
Also load the mammals sleep dataset from the [VIM package](https://cran.r-project.org/web/packages/VIM/VIM.pdf) that is located in the data folder. This dataset looks at sleep patterns in various mammal species and contains missing (`NA`) values. See the link below for the original research article:
```{r}
library(VIM)
data(sleep) 
```
