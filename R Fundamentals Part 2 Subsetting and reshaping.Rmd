...
# 5.  Merging data
Merging data is useful when we want to combine two different dataframes that share a vector/column. We "merge" by that column. The first two arguments in `merge()` are the names of the two data frames, followed by `by` where we tell which column names we want to match:For example: 
```{r, eval=FALSE}
...
`cbind()` will bind two data frames by their columns. An example:
```{r, eval=FALSE}
?cbind
...
df2 <- data.frame(Name=c("Jack", "Susan", "Joe", "Kelly"),
                  DOB=c("March", "December", "January", "July"))
df2
...
df5 <- data.frame(Name=c("Heather", "Billy"),
                  City=c("San Francisco", "San Francisco"),
                  Math=c(49, 44),
                  Reading=c(10,10))
df5
