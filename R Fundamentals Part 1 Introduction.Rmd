...
It is a good idea to start off the class by explpaining to folks why you (the instructor) use R. Also describe in general why social scientists use R as well as some of its pitfalls. Then, break the ice by going around the room and asking each student to state their name, department, and why they want to learn R. Common responses include:
...
You can [download R Studio here](https://www.rstudio.com/products/rstudio/download/)

If this installation says that you are lacking the binary files, [download R binary files here](https://cran.r-project.org/mirrors.html)

After opening R Stuio, by default you should see four windows after opening a new script:
...
```
Let's try another example, this time using character data.
Note that character data is ***always*** contained within quotation marks `" "` 
```{r}
...
animals$Name # Now when we call animals$Name, we can see that the levels have changed
```
...
1. Vectors
  ...
  5. Create a sentence from separate words using `paste()`. Can you guess how to add another word to this vector without creating a new vector?

2. Lists
  1. How does a list differ from an atomic vector?
  2. Create three objects of different types and lengths and then combine them into a list names `x`.
  3. If `x` is a list, what is the class of `x[1]`? How about `x[[1]]`? (this is a preview of Part 2).

3. Data frames
  1. Create a 3x2 data frame called `basket`. List the name of each fruit in the first column and its price in teh second column. 
  2. Now give your dataframe appropriate column and row names. 
  3. We can add a new column using `$` (this will be covered with data subsetting in Part 2). Can you guess how to add a third column called "Color", that shows the color of each fruit?
