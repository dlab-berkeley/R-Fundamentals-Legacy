...
- Visit [https://github.com/dlab-berkeley/R-Fundamentals](https://github.com/dlab-berkeley/R-Fundamentals) to download the course materials and save them to your desktop. You can either:  

1. click the green `download zip` button on the right hand side of the screen, click `download zip`, and extract this zip file someplace convenient, or  

2. type `git clone git@github.com:dlab-berkeley/R-Fundamentals.git` into your terminal.  
...
Remember, our goal is to teach you responsible computational programming with R - not statistics or math. To learn statistics and math, check out UC Berkeley:  
...
RStudio is an R graphical user interface (GUI) and integrated development environment (IDE) that permits the user to enter and save code and data for data preparation, exploration, plotting, and testing.  
...
**Save your script** by clicking "File" then "Save", or `command + s` (Mac) or `Ctrl + s` (PC). You can also click the 

### R markdown
This is an R markdown (.Rmd) file. It allows you create documents of various kinds. Check out the [R Markdown Quick Tour](http://rmarkdown.rstudio.com/authoring_quick_tour.html). Some highlights:
1. Enter text as normal!  
2. Code must be entered as "chunks" that look like this:  
```{r}
2 + 2
# but hashtag text within a chunk to add a comment to it.
```
...
5. Click the "Run" button (top right portion of your R Markdown window) and "Run All" to run all! 
...
### Function arguments
Arguments go inside of the parentheses of R commands. Unnamed arguments are things like the number 4. `split = " "` is what is called a named argument. This argument goes inside the parentheses of another command such as `strsplit`. 
```{r}
...
Most commands require one or two arguments to be defined in order for the command to properly run. You will find that commands are full of default, positional, and optional arguments that you can manipulate.  
