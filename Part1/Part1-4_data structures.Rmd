...
2. `c()` - concatenate vectors  
3. `list()` - create lists  
4. `matrix()` - construct matrices  
5. `data.frame()` - combine vectors into a structured, tabular spreadsheet with rows and columns.  
...
`split = " "` is what is called a named argument. This argument goes inside the parentheses of another function such as `strsplit`.

```{r}
# Example of named arguments 
addTwoNum <- function(first, second) {first + second}
...
# Full matching (named arguments) 
addTwoNum(first = 2, second = 3)
```
...
```
How did you do? Counting out all the character positions (indexes or indices) is a pain! Thankfully there are functions we can use to search for words. Note that our pangram is no longer a pangram!
...
A **VECTOR** is an ordered group of the same kind of data. "Ordered" means that position matters. Vectors are one-dimensional and homogeneous, and are thus referred to by their type (e.g., character vector, numeric vector, logical vector). It does not matter what type of data are in the vector, as long as it is all the same! 
