...
Part 2) Import, subset, and merge data; type coercion, identifying missing data  
Part 3) Research design, data visualization, and statistical testing  
...
R uses `+` when code is broken up across multiple lines and R is still expecting more code. A line of code does not usually stop until R finds an appropriate stop parameter, or some punctuation that completes some code such as a closed round parenthesis `)`, square bracket `]`, curly brace `}`, or quotation mark `'`.  
...
Data are saved in R's memory as "variables". Variables are simply placeholders for a value, word, function, or dataset! The global "Environment" tab in the upper right pane displays the variables you have assigned. "Global" simply means that these variables are available for any task.  
...
* Arguments are the things; they consist of virtually anything like values, mathematical expressions, vectors, lists, matrices, words, text of entire books, functions, and even large datasets. These arguments go inside the parentheses of functions!

```{r}
mean(c(100, 1000, 10000))
```

Flash question: what are functions and arguments? 
