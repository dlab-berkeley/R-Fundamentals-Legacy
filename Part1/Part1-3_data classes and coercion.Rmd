...
Everything in R has a class (or "type" in Python; to be precise, class is a [blueprint for creating a particular data structure](https://brilliant.org/wiki/classes-oop/#:~:text=In%20object%2Doriented%20programming%2C%20a,created%20using%20the%20class%20keyword.)). This is important because it determines what we can do to that thing! **The five most common data classes are: numeric, integer, character, logical and factor.** Use the `class` function to check a variable's type. 
...
`c` is the function to concatenate (combine) a vector of data (see "Vector" section below).  

`NA` is how R recognizes explicit missing data (see Part 2 materials).  
