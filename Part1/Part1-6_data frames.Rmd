---
title: 'R Fundamentals Part1-6: data frames'
author: "D-Lab"
...
_**It is worth emphasizing the importance of data frames in R!**_ Inside R, a **DATA FRAME** is a list of equal-length vectors. These vectors can be of different types. Data frames are thus heterogenous like lists. Data frames have the **same type of data within each column**, but **different types of data across columns!**

This is simply a tabular spreadsheet of rows and columns!  
...
# NOTE: `stringsAsFactors=FALSE` means that R will NOT automatically coerce character vectors to factor type. 
```
...
We can change the names of the columns by passing into it a vector with our desired names of equal length to the existing column names (five): 
...
For example, in our `gdp` dataframe, we can coerce "Country" from character to factor type. Canada, Mexico, and USA are the factor levels. If we want to compare `gdpPercap` and `growth_rate` between these three countries, we set the character "Country" vector to factor data type. We can do so with `factor()`.  
...
Flash question: where have you seen something that looks like a dataframe before? 

See (this page)[https://www.stat.berkeley.edu/~s133/factors.html] for help with factors.  
